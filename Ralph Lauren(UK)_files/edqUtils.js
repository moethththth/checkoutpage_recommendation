...
function setEdqInputSelectors(stageContentLocation) {
	...
	if (stageContentLocation === "shipping") {
		...
		edqAddressLine1Id = document.querySelector("#dwfrm_profile_address_address1");
		edqAddressLine2Id = document.querySelector("#dwfrm_profile_address_address2");
		edqCityLineId = document.querySelector("#dwfrm_profile_address_city");
		edqPostalLineId = document.querySelector("#dwfrm_profile_address_postal");
		edqStateLineId = document.querySelector("#dwfrm_profile_address_states_state");
		edqCountryLineId = document.querySelector("#dwfrm_profile_address_country");
	}
...
function edqSetGlobalIntuitiveConfiguration() {
	 ...
	 */
	if (edqDataSetUsage && (edqDataSetCode !== null && edqDataSetCode !== '' && edqDataSetCode !== 'null')) {
...
// Add this function to update the fields after user select the address from the list due to Experian has no called back function.
var isIntervalStart = false, triggerNumber = 0;
	...
	function startFunction() {
		try { 
			if(!$('.edq-global-intuitive-address-suggestions').is(":visible") && $('.edq-global-intuitive-address-suggestions').find('.selected').length > 0){
				stopFunction();
				var $hiddenFields = $('.shipping-address-block').find('.detailed-shipping-info-formfields');
				if ($hiddenFields.hasClass('d-none')) {
					$hiddenFields.removeClass('d-none');
				}
				setTimeout(function(){
					var $address1 = $('#shippingAddressOnedefault, #dwfrm_profile_address_address1');
					var $address2 = $('#shippingAddressTwodefault, #dwfrm_profile_address_address2');
					var $city = $('#shippingAddressCitydefault, #dwfrm_profile_address_city');
					var $postcode = $('#shippingZipCodedefault, #dwfrm_profile_address_postal');
					var $state = $('#shippingStatedefault, #dwfrm_profile_address_states_state, #dwfrm_profile_address_states_stateCode'); 
					if(!validateZipCode($postcode.val()) && edqCountryLineId.value == 'US') {
						$postcode.val($postcode.val().substring(0,5));
					}
					var $experianSuggest = $('#experianSuggest');					
					$experianSuggest.val(true);
					if(edqCountryLineId.value == 'CA') {
						$address1.val(convertCapitalized($address1.val().substring(0,35)));
						$address2.val(convertCapitalized($address2.val().substring(0,35)));
						$city.val(convertCapitalized($city.val()));
					} else {
						$address1.val($address1.val().substring(0,35));
						$address2.val($address2.val().substring(0,35));
					}
					if($state[0].id == 'dwfrm_profile_address_states_stateCode'){
						var formatdata = $address1.attr('edq-metadata');
						if(formatdata){
							var addressJson = JSON.parse(formatdata);
							$state.val(addressJson.address[4].province).change();
						}
...
edqAddressLine1Id.addEventListener("input", updateFormField, true);